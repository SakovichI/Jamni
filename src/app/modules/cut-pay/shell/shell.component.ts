...
export class ShellComponent implements OnInit, OnDestroy{
  ...
  public pay(): void{
    ...
    this.apiOrderService.makeOrder({
      deliveryType: 'COURIER',
      email: userForm.email,
      discountCode: 'no',
      payType: 'CASH',
      address: {"address": [userForm.phone+ ' ' + userForm.street + ' ' + userForm.house, userForm.index, userForm.city, 'Россия'].join(', ')},
      items: cart.map((v: any) => ({"id": v.product.id, "amount": v.amount, "specificationValues": v.appliedSpecs}))
    }).subscribe(() => {
